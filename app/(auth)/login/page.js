...
export default function LoginPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (remember && refresh_token) localStorage.setItem("refresh_token", refresh_token);
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} required value={password}
                ...
                className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none shadow-sm" />
              <span className="absolute right-4 top-[42px] text-slate-500 cursor-pointer"
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-sm text-slate-600">
                <input type="checkbox" checked={remember}
                  onChange={() => setRemember(!remember)} className="h-4 w-4" />
                Remember me
              </label>

              <Link href="/forgot-password" className="text-sm text-slate-900 font-medium hover:underline">
                Forgot password?
              </Link>
            </div>

            <button type="submit" disabled={loading}
              className="w-full py-3 rounded-full bg-slate-900 hover:bg-slate-800 text-white font-semibold shadow-md transition disabled:opacity-50">
